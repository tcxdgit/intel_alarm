...
def parse_event(event_json):
    ...
    if module == 'BFD':
        if log_type_desc == "BFD_CHANGE_FSM":
            # [ ]要转义
            patterns = ["Sess\[%{DATA:sess}\], Ver(%{DATA})?, Sta: %{DATA:initial_status}->%{DATA:status}, Diag: %{DATA}"]
            ...
            m_sess = Grok("%{DATA:session}, LD/RD:%{DATA:LD_RD}, Interface:%{DATA:interface}, SessType:%{DATA:sess_type}, LinkType:%{DATA:link_type}").match(sess)
        ...
        if log_type_desc == "HA_STANDBY_TO_MASTER":
        ...
        if log_type_desc == "LLDP_DELETE_NEIGHBOR":
            patterns = ["%{DATA} agent neighbor deleted on port %{DATA:port} \(IfIndex %{NUMBER:if_index}\), neighbor's chassis ID is %{DATA:neighbor_chassis_id}, port ID is %{DATA:neighbor_port_id}\."]
        ...
        elif log_type_desc == "OSPFv3_LAST_NBR_DOWN":
            patterns = ["OSPFv3 %{NUMBER:ospfv3Id} Last neighbor down event: Router ID: %{DATA:neighbor}  Local interface ID: %{NUMBER:local_interface_id}  Remote interface ID: %{NUMBER:remote_interface_id}  Reason: %{DATA}."]
        ...
        if log_type_desc == "STP_NOTIFIED_TC":
        ...
        if log_type_desc == "LAGG_INACTIVE_OTHER":
            ...
            patterns = ["Member port %{DATA:member_port} of aggregation group %{DATA:port} changed to the inactive state, because the physical or line protocol state of the port was down."]
            ...
            result['level'] = len(result['NE'])
            result['abstract'] = "Member port {} of aggregation group {} changed to the inactive state".format(member_port, raw_port)
            return result
